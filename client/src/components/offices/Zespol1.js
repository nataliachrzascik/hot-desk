    ...
    const makeReservationUser=()=>{
        props.dispatch(makeReservation(`/reservation/${tableNumber}/${year}${month}${day}/${userID}`))
          .then((res)=>{
            ...
            if(res.data==="Ktoś prawdopodobnie ubiegł Cię w rezerwacji miejsca!"){
            ...
            else if(res.data==="Użytkownik może mieć tylko 1 rezerwacje dziennie"){
              document.getElementById('popup').style.visibility = "hidden";
              document.getElementById('popupEr').style.visibility = "visible";
              //alert("Użytkownik może mieć tylko 1 aktywną rezerwacje dziennie. Możesz usunąć aktywną rezerwacje na wybrany dzień w panelu użytkownika.");
              return
            }
    ...
    return (
        ...
        <div id="13" className="table Place13Wro2 zespol1" onClick={()=>chooseDesk(13)}/>
            ...
            <Paper elevation={3} id="popup" className="visibility" >
                ...
                <Paper elevation={3} id="popupEr" className="visibilityEr" >
                <Button variant="contained" style={{"float":"right"}} onClick={()=> makeITErDissapear()}><span class="bolder">X</span></Button>
                <h1 class="popupErText">{`Użytkownik może mieć tylko 1 aktywną rezerwacje dziennie. Możesz usunąć aktywną rezerwacje na wybrany dzień w panelu użytkownika.`}</h1>
                <Button variant="contained" onClick={()=> makeITErDissapear()}>Ok</Button>
                </Paper>
            </>
